...
async function consume(env, input) {
  const command = safe(input.command).toLowerCase(); const policy = await env.DB.prepare('SELECT * FROM commands WHERE name=?').bind(command).first();
...
async function mutateAdmin(env, actor, action, input, request) {
  ...
  else if(action==='command'){ before=await env.DB.prepare('SELECT * FROM commands WHERE name=?').bind(input.name).first(); await env.DB.prepare('UPDATE commands SET enabled=?,premium_only=?,free_limit=?,premium_limit=?,group_limit=?,member_limit=?,updated_at=? WHERE name=?').bind(input.enabled?1:0,input.premiumOnly?1:0,input.freeLimit??null,input.premiumLimit??null,input.groupLimit??null,input.memberLimit??null,now(),input.name).run(); after=await env.DB.prepare('SELECT * FROM commands WHERE name=?').bind(input.name).first(); }
