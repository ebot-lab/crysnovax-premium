...
async function touchSubjects(env, userId, chatId) { const stamp = now(); const statements = [env.DB.prepare("INSERT INTO subjects(scope,telegram_id,display_name,username,metadata,updated_at) VALUES('user',?,NULL,NULL,'{}',?) ON CONFLICT(scope,telegram_id) DO UPDATE SET updated_at=excluded.updated_at").bind(String(userId), stamp)]; if (chatId) statements.push(env.DB.prepare("INSERT INTO subjects(scope,telegram_id,display_name,username,metadata,updated_at) VALUES('group',?,NULL,NULL,'{}',?) ON CONFLICT(scope,telegram_id) DO UPDATE SET updated_at=excluded.updated_at").bind(String(chatId), stamp)); await env.DB.batch(statements); }
async function consume(env, input) {
  const command = safe(input.command).toLowerCase();
  await touchSubjects(env, input.userId, input.chatId);
