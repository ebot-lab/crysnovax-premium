...
async function passwordLogin(request, env) {
  ...
  const windowStart = Date.now() - 9e5;
  ...
  if (!valid) {
    await audit(env, "anonymous", "password_login_failed", "ip", ip, null, null, null, null);
    throw new ApiError("Invalid password", 401);
  }
  const session = id();
  const csrf = id();
  await env.DB.prepare("INSERT OR IGNORE INTO admins VALUES(?,?,?,?)").bind(adminId, "owner", "Admin", now()).run();
  await env.DB.prepare("INSERT INTO sessions VALUES(?,?,?,?,?)").bind(session, adminId, csrf, new Date(Date.now() + 6048e5).toISOString(), now()).run();
...
// .wrangler/tmp/bundle-gn3my2/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gn3my2/middleware-loader.entry.ts
